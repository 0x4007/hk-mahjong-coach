...
Buildings begin as closed rectangular masses. The runtime decoration pass adds deterministic facade windows and an
external fire-stair stairwell with a landing at every generated floor and a final landing at the roof. The stairwell is
an exterior traversal feature; interiors and doors remain out of scope.
...
The v0.1 implementation additionally derives:

- Repeated windows on all four facades, keyed by building, floor, column, and facade.
- Repeated stair flights with deterministic direction reversal at each landing, plus a final roof-edge bridge that
  overlaps the building footprint at roof height.
- A sloped physics box per flight and a thin landing collider, while the building mass remains the coarse blocker.

There are no accessible doors or interiors; rooftop access is provided by the exterior stairwell.
...
- Do not create one draw call per building, sidewalk section, window, stair step, or cover item.
...
- Render windows and stair steps through shared instanced batches; do not create one mesh per detail.
