...
const conceptsFromFacts = (facts: readonly AnalysisFact[]): readonly ConceptId[] => {
  ...
  for (const fact of facts) {
    let concept: ConceptId | undefined;
    switch (fact.kind) {
      case "distance":
      case "score_gap":
        concept = "tile_efficiency";
        break;
      case "improving_tiles":
        concept = "waits_improving_tiles";
        break;
      case "visible_copies":
        concept = "visible_tile_counting";
        break;
      case "faan_path":
        concept = "minimum_faan_planning";
        break;
      case "relative_risk":
        concept = "relative_safety";
        break;
      case "legal_rule":
        concept = "turn_order_claim_priority";
        break;
      case "learner_pattern":
        concept = "tile_efficiency";
        break;
      default:
        concept = undefined;
...
const isConceptId = (value: string): value is ConceptId => CONCEPT_IDS.includes(value as ConceptId);
...
const decisionFromRecord = (
  ...
  if (
    ...
    !["none", "nudge", "compare", "reveal"].includes(hintLevel)
  ) {
    return null;
  }
  return {
...
export class SessionController {
  ...
  public createDrillSession(learnerId: string, conceptIds: readonly ConceptId[] = []) {
    const library = createBundledDrillLibrary();
    const mastery = this.mastery(learnerId);
    const weakConcept = [...mastery]
      .filter(({ mastery: value }) => value < 0.75)
      .sort(
        (left, right) =>
          left.mastery - right.mastery || left.conceptId.localeCompare(right.conceptId),
      )
      .at(0)?.conceptId;
    const curriculumConcept = nextCurriculumConcept(mastery);
    const preferredConcepts =
      conceptIds.length > 0
        ? [...conceptIds]
        : [weakConcept ?? curriculumConcept].filter(
            (concept): concept is ConceptId => concept !== null,
          );
    const dueConcepts = new Set(
      mastery
        .filter(({ nextReviewAt }) => nextReviewAt !== null && nextReviewAt <= this.#clock())
    ...
    const items = library
      .filter(
        (item) =>
          ...
          item.conceptIds.some((concept) => preferredConcepts.includes(concept)),
      )
      .sort(
        (left, right) =>
          Number(right.conceptIds.some((concept) => dueConcepts.has(concept))) -
            Number(left.conceptIds.some((concept) => dueConcepts.has(concept))) ||
    ...
    const selectedConceptIds = [
      ...new Set(items.flatMap(({ conceptIds: itemConcepts }) => itemConcepts)),
    ].sort();
    const sessionId = `drill-session:${canonicalJsonHash({ learnerId, conceptIds: selectedConceptIds, itemIds: items.map(({ id }) => id) }).slice(7, 39)}`;
    const repository = this.repositoryFor();
    repository.ensureLearner(learnerId);
  ...
  public answerDrill(input: SessionDrillAnswerInput): SessionDrillAnswerResult {
    const session =
      this.#drillSessions.get(input.sessionId) ?? this.restoreDrillSession(input.sessionId);
  ...
  private restoreDrillSession(sessionId: string): DrillSession | undefined {
    const exported = this.repositoryFor().exportData({ includeLlmMetadata: false });
    const library = createBundledDrillLibrary();
    const records = exported.data.drillItems
      .filter(({ data }) => data.sessionId === sessionId)
      .sort((left, right) => {
        const leftIndex = left.data.sessionIndex;
        const rightIndex = right.data.sessionIndex;
        return (
          (typeof leftIndex === "number" ? leftIndex : Number.MAX_SAFE_INTEGER) -
            (typeof rightIndex === "number" ? rightIndex : Number.MAX_SAFE_INTEGER) ||
          left.id.localeCompare(right.id)
        );
      });
    const first = records[0];
    if (first === undefined) return undefined;
    const itemById = new Map(library.map((item) => [item.id, item] as const));
    const recordIds = new Set(records.map(({ id }) => id));
    const attempted = new Set(
      exported.data.drillAttempts
        .filter(({ drillItemId }) => recordIds.has(drillItemId))
        .map(({ drillItemId }) => drillItemId),
    );
    const remaining = records
      .filter(({ id }) => !attempted.has(id))
      .map(({ id }) => itemById.get(id))
      .filter((item): item is DrillItem => item !== undefined);
    const items =
      remaining.length > 0
        ? remaining
        : records
    ...
    const session = { learnerId: first.learnerId, items };
    this.#drillSessions.set(sessionId, session);
    return session;
  }
  ...
  private persist(
    ...
    const repository = this.repositoryFor();
    const persisted = repository.appendAcceptedCommand({
      key: game.key,
      requestId,
      events,
      state,
    ...
    if (decisionEvidence !== undefined && persisted.disposition === "appended") {
      this.persistDecisionMastery(game, decisionEvidence);
    }
    this.persistPostHandReview(game, state);
  }

  private persistDecisionMastery(game: SessionGame, evidence: AcceptedDecisionEvidenceInput): void {
    const conceptIds = stringArray(evidence.decision.data.conceptIds).filter(isConceptId);
    const hintValue = evidence.decision.data.hintLevel;
    const hintLevel: HintLevel =
      hintValue === "nudge" || hintValue === "compare" || hintValue === "reveal"
        ? hintValue
        : "none";
    const repository = this.repositoryFor();
    const occurredAt = this.#clock();
    for (const conceptId of conceptIds) {
      const previous =
        this.mastery(game.learnerId).find(({ conceptId: current }) => current === conceptId) ??
        null;
      const update = updateConceptMastery(previous, {
        learnerId: game.learnerId,
        conceptId,
        quality: evidence.decision.quality ?? 0,
        independent: evidence.decision.independent,
        hintLevel,
        occurredAt,
      });
      const mastery = update.mastery;
  ...
  private decisionEvidenceFor(
    ...
    const selectedAction = analyzed.find(({ actionId: candidateId }) => candidateId === actionId);
    const selected = analysis.candidates.find(
      ({ actionId: candidateId }) => candidateId === actionId,
    );
    // Starting the next hand is a lifecycle command, not a learner decision. It has no
    // corresponding discard/claim/kong/win event for the evidence bridge to validate.
    if (selected === undefined || selectedAction?.actionType === "start_next_hand")
      return undefined;
    const exported = this.repositoryFor().exportData({ includeLlmMetadata: false });
