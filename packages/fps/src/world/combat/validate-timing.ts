...
const shortestPath = (
  ...
  while (visited.size < nodes.length) {
    ...
    for (const edge of plan.edges) {
      const neighbor = edge.fromNodeId === current ? edge.toNodeId : edge.toNodeId === current ? edge.fromNodeId : null;
...
export const validateTiming = (
  ...
  const attackerMean = Math.max(0.001, (metrics.attackerToASeconds + metrics.attackerToBSeconds) / 2);
  if (attackerDifference / attackerMean > 0.2) {
    failures.push({ code: "route-time", message: "attacker route times differ by more than 20%", nodeIds: ["attacker-spawn", "objective-a", "objective-b"] });
  }
  ...
  if (defenderAdvantageA < 0 || defenderAdvantageA > 15 || defenderAdvantageB < 0 || defenderAdvantageB > 15) {
    failures.push({ code: "defender-advantage", message: "defender arrival advantage is outside the playable 0–15 second envelope", nodeIds: ["attacker-spawn", "defender-spawn", "objective-a", "objective-b"] });
  }
  if (metrics.siteToSiteSeconds < 12 || metrics.siteToSiteSeconds > 40) {
    failures.push({ code: "site-rotation", message: "site-to-site rotation is outside the playable 12–40 second envelope", nodeIds: ["objective-a", "objective-b"] });
  }
  if (metrics.firstContactSeconds < 8 || metrics.firstContactSeconds > 25) {
    failures.push({ code: "route-time", message: "first-contact route is outside the 8–25 second envelope", nodeIds: ["attacker-spawn", "arena"] });
  }
  return { valid: failures.length === 0, failures, metrics };
};

export const directDistance = (plan: Pick<CombatDistrictPlan, "nodes">, from: string, to: string): number =>
  distance2(findNode(plan.nodes, from).position, findNode(plan.nodes, to).position);
