...
import { chunkBounds, expandBounds, intersectsBounds, isValidChunkCoord } from "../coordinates.js";
import { BUILDING_DETAIL_VERSION } from "../planning/generate-building-details.js";
import type {
  AxisAlignedSegment,
  Bounds2,
  BuildingPart,
...
const clipPolygon = (points: readonly Vec2[], bounds: Bounds2): readonly Vec2[] => {
  ...
  clip(
    ...
    (left, right) => ({
      x: bounds.minX,
      z: left.z + ((right.z - left.z) * (bounds.minX - left.x)) / (right.x - left.x),
    }),
  );
  clip(
    ...
    (left, right) => ({
      x: bounds.maxX,
      z: left.z + ((right.z - left.z) * (bounds.maxX - left.x)) / (right.x - left.x),
    }),
  );
  clip(
    (point) => point.z >= bounds.minZ,
    (left, right) => ({
      x: left.x + ((right.x - left.x) * (bounds.minZ - left.z)) / (right.z - left.z),
      z: bounds.minZ,
    }),
  );
  clip(
    ...
    (left, right) => ({
      x: left.x + ((right.x - left.x) * (bounds.maxZ - left.z)) / (right.z - left.z),
      z: bounds.maxZ,
    }),
  );
  return current;
...
const containsHalfOpen = (bounds: Bounds2, point: Vec2): boolean =>
  point.x >= bounds.minX &&
  point.x < bounds.maxX &&
  point.z >= bounds.minZ &&
  point.z < bounds.maxZ;
...
const sliceSurface = (
  ...
  return points.length < 3
    ? null
    : { ...surface, id: `${surface.id}@${String(coord.x)}:${String(coord.z)}`, points };
};
...
const sliceBuilding = (
  ...
  return clipped === null
    ? null
    : { ...part, id: `${part.id}@${String(coord.x)}:${String(coord.z)}`, bounds: clipped };
};
...
const sliceCover = (
  ...
  return clipped === null
    ? null
    : { ...cover, id: `${cover.id}@${String(coord.x)}:${String(coord.z)}`, bounds: clipped };
};

const featureBounds = (feature: CombatFeature): Bounds2 => {
  if (feature.kind === "node" || feature.kind === "objective" || feature.kind === "spawn") {
    return {
      minX: feature.position.x,
      maxX: feature.position.x,
      minZ: feature.position.z,
      maxZ: feature.position.z,
    };
  }
...
const combatFeatures = (plan: WorldPlan, bounds: Bounds2): readonly CombatFeature[] => {
  ...
  for (const node of district.nodes) {
    if (containsHalfOpen(bounds, node.position))
      features.push({ kind: "node", id: node.id, nodeKind: node.kind, position: node.position });
  }
  for (const edge of district.edges) {
    if (
      edge.segments.some((segment) => {
        const segmentFeatureBounds = featureBounds({
          kind: "edge",
          id: edge.id,
          routeRole: edge.routeRole,
          spaceKind: edge.spaceKind,
          widthM: edge.widthM,
          segments: [segment],
        });
        return intersectsBounds(expandBounds(segmentFeatureBounds, edge.widthM / 2), bounds);
      })
    ) {
      features.push({
        kind: "edge",
  ...
  for (const cover of district.coverObjects) {
    if (intersectsBounds(cover.bounds, bounds))
      features.push({
        kind: "cover",
        id: cover.id,
  ...
  for (const node of district.nodes) {
    if (node.kind === "objective-a" || node.kind === "objective-b") {
      if (containsHalfOpen(bounds, node.position))
        features.push({ kind: "objective", id: node.id, position: node.position });
    }
    if (node.kind === "attacker-spawn" || node.kind === "defender-spawn") {
      if (containsHalfOpen(bounds, node.position))
        features.push({
          kind: "spawn",
          id: node.id,
          team: node.kind === "attacker-spawn" ? "attacker" : "defender",
...
const edgeSignature = (
  ...
  const features = [
    ...
    ...plan.sidewalks.map((surface) => ({
      id: surface.id,
      kind: "sidewalk",
      points: surface.points,
    })),
    ...plan.buildingParts.map((part) => ({ id: part.id, kind: part.kind, bounds: part.bounds })),
    ...plan.combatDistrict.coverObjects.map((cover) => ({
      id: cover.id,
      kind: "cover",
      bounds: cover.bounds,
    })),
  ].filter((feature) => {
    const featureBoundsValue =
      "bounds" in feature
        ? feature.bounds
        : "points" in feature
          ? {
              minX: Math.min(...feature.points.map((point) => point.x)),
              maxX: Math.max(...feature.points.map((point) => point.x)),
              minZ: Math.min(...feature.points.map((point) => point.z)),
              maxZ: Math.max(...feature.points.map((point) => point.z)),
            }
          : {
              minX: Math.min(feature.start.x, feature.end.x),
              maxX: Math.max(feature.start.x, feature.end.x),
              minZ: Math.min(feature.start.z, feature.end.z),
              maxZ: Math.max(feature.start.z, feature.end.z),
            };
    return axis === "x"
      ? Math.abs(featureBoundsValue.minX - coordinate) < 1e-6 ||
          Math.abs(featureBoundsValue.maxX - coordinate) < 1e-6
      : Math.abs(featureBoundsValue.minZ - coordinate) < 1e-6 ||
          Math.abs(featureBoundsValue.maxZ - coordinate) < 1e-6;
  });
  void bounds;
  void coord;
...
export const deriveChunkPlan = (plan: WorldPlan, coord: ChunkCoord): ChunkPlan => {
  if (!isValidChunkCoord(plan.config, coord))
    throw new Error("world_chunk_coordinate_out_of_bounds");
  const bounds = chunkBounds(plan.config, coord);
  const haloBounds: Bounds2 = {
    minX: Math.max(plan.bounds.minX, bounds.minX - plan.config.layoutCellSizeM),
    maxX: Math.min(plan.bounds.maxX, bounds.maxX + plan.config.layoutCellSizeM),
    minZ: Math.max(plan.bounds.minZ, bounds.minZ - plan.config.layoutCellSizeM),
    maxZ: Math.min(plan.bounds.maxZ, bounds.maxZ + plan.config.layoutCellSizeM),
  };
  const roads = plan.roads
    .map((road) => sliceRoad(road, bounds, coord))
    .filter((road): road is RoadSegmentSlice => road !== null);
  const sidewalks = plan.sidewalks
    .map((surface) => sliceSurface(surface, bounds, coord))
    .filter((surface): surface is SurfacePolygon => surface !== null);
  const buildingParts = plan.buildingParts
    .map((part) => sliceBuilding(part, bounds, coord))
    .filter((part): part is BuildingPart => part !== null);
  const buildingWindows = plan.buildingWindows.filter((window) =>
    containsHalfOpen(bounds, window.position),
  );
  const buildingStairwells = plan.buildingStairwells.filter((stairwell) =>
    containsHalfOpen(bounds, stairwell.ownerPosition),
  );
  const coverObjects = plan.combatDistrict.coverObjects
    .map((cover) => sliceCover(cover, bounds, coord))
    .filter((cover): cover is CoverPlacement => cover !== null);
  ...
  const hashable = {
    ...
    buildingWindowIds: buildingWindows.map((window) => window.id),
    buildingStairwellIds: buildingStairwells.map((stairwell) => stairwell.id),
    coverObjects,
