...
const keepInside = (bounds: Bounds2, outer: Bounds2): Bounds2 => {
  ...
  const centerX = clamp((bounds.minX + bounds.maxX) / 2, outer.minX + width / 2, outer.maxX - width / 2);
  const centerZ = clamp((bounds.minZ + bounds.maxZ) / 2, outer.minZ + depth / 2, outer.maxZ - depth / 2);
...
export const generateBuildings = (
  ...
  return parcels.map((parcel, index): BuildingPart => {
    ...
    const width = clamp(
      widthAvailable * (0.72 + random.randomFloat("building-width", parcel.bounds.minX, parcel.bounds.minZ) * 0.18),
      Math.min(15, maxWidth),
      Math.min(40, maxWidth),
    );
    const depth = clamp(
      depthAvailable * (0.72 + random.randomFloat("building-depth", parcel.bounds.maxX, parcel.bounds.maxZ) * 0.18),
      Math.min(15, maxDepth),
      Math.min(40, maxDepth),
    );
    const center = {
      x: (parcel.bounds.minX + parcel.bounds.maxX) / 2 +
        (random.randomFloat("building-offset-x", parcel.bounds.minX, parcel.bounds.minZ) - 0.5) * Math.max(0, widthAvailable - width - inset * 2),
      z: (parcel.bounds.minZ + parcel.bounds.maxZ) / 2 +
        (random.randomFloat("building-offset-z", parcel.bounds.maxX, parcel.bounds.maxZ) - 0.5) * Math.max(0, depthAvailable - depth - inset * 2),
    };
    ...
    return {
      ...
      kind: "building",
      bounds,
      heightM: 10 + random.randomFloat("building-height", parcel.bounds.minX, parcel.bounds.maxZ) * 30,
